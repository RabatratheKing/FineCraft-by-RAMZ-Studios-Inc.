...
# I will find the lines starting at `float uv0[2]` down to the end of the `if(!flip)` block and replace it.
import re
pattern = re.compile(r'float uv0\[2\].*?addV\(v0, 0, fh\);\s*\}', re.DOTALL)
content = pattern.sub(uv_logic.strip(), content)

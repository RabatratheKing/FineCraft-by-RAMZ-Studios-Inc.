...
int main() {
    ...
    std::unordered_map<uint16_t, int> blockToTex;
    std::unordered_map<uint16_t, std::string> blockToName;
    ...
    if (!blocksStr.empty()) {
        try {
            ...
            for (auto& el : j.items()) {
                ...
                blockToTex[block_id] = texAll;
                blockToName[block_id] = identifier;
            }
        } catch (...) {}
    }
    
    std::cout << "Stone (3) tex: " << blockToTex[3] << " (name: " << blockToName[3] << ")" << std::endl;
    std::cout << "Dirt (2) tex: " << blockToTex[2] << " (name: " << blockToName[2] << ")" << std::endl;
    std::cout << "Grass (1) tex: " << blockToTex[1] << " (name: " << blockToName[1] << ")" << std::endl;
    std::cout << "Log (4) tex: " << blockToTex[4] << " (name: " << blockToName[4] << ")" << std::endl;
    std::cout << "Sand (8) tex: " << blockToTex[8] << " (name: " << blockToName[8] << ")" << std::endl;

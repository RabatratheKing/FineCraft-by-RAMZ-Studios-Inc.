...
uv_logic = """
                        ...
                        if (d == 0) { // X axis. u=Y(fw), v=Z(fh)
                            ...
                            if (dir == 1) { // +Y Top
                                ...
                                // Look vector = +Y (Up). Up = +Z (South). Right = +X (East).
                                // v0 (smallest Z, smallest X).
                                // Up is +Z, so v0 is BOTTOM.
                                // Right is +X, so v0 is LEFT.
                                // v0 = Bottom-Left -> UV(0, fh)
                                // v1 = v0 + Z (UP) -> Top-Left -> UV(0, 0)
                                // v3 = v0 + X (RIGHT) -> Bottom-Right -> UV(fw, fh)  // wait, fw is Z, fh is X
                                // No, U is horizontal (fw), V is vertical (fh). Wait, U is along fw? No, U is horizontal.
                                // Let's just map Bottom same as Top but flipped.
                                uv0[0] = fw; uv0[1] = fh;
                                uv1[0] = fw; uv1[1] = 0;
                                uv2[0] = 0; uv2[1] = 0;
                                uv3[0] = 0; uv3[1] = fh;
